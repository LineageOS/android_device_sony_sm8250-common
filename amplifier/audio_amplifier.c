...
static int get_ta_array(uint32_t unit, void *arr, bool reverse) {
  ...
  if (ret) {
    ALOGE("%s: Cannot read TS unit %d of size %u: error %d", __func__, unit,
          ta_sz, ret);
    goto end;
  }
...
#ifdef TA_DEBUG
  ALOGI("%s: Read TA unit %u (size %u) values: 0x%x 0x%x 0x%x 0x%x", __func__,
        unit, ta_sz, array[0], array[1], array[2], array[3]);
#endif
  return ret;
}
...
static int cirrus_set_mixer_value_by_name_lr(char *ctl_base_name, int value) {
  ...
  ret =
      cirrus_format_mixer_name(ctl_base_name, "L", ctl_name, sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret =
      cirrus_format_mixer_name(ctl_base_name, "R", ctl_name, sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ret = cirrus_set_mixer_value_by_name(ctl_name, value);
  if (ret < 0)
...
static int cirrus_get_mixer_value_by_name(char *ctl_name) {
  ...
  if (ret < 0)
    ALOGE("%s: Cannot get mixer %s value: error %d", __func__, ctl_name, ret);
exit:
  mixer_close(card_mixer);
  return ret;
}

static int cirrus_set_mixer_array_by_name(char *ctl_name, void *array,
                                          size_t count) {
  struct mixer *card_mixer = NULL;
  struct mixer_ctl *ctl_config = NULL;
  int sndcard_id = 0, ret = 0;
  ...
  if (ret < 0)
    ALOGE("%s: Cannot set mixer %s", __func__, ctl_name);
exit:
  mixer_close(card_mixer);
  return ret;
}
...
static int cirrus_set_mixer_enum_by_name(char *ctl_name, const char *value) {
  ...
  if (ret < 0)
    ALOGE("%s: Cannot set mixer '%s' to '%s'", __func__, ctl_name, value);
exit:
  mixer_close(card_mixer);
  return ret;
}
...
static int cirrus_play_silence(int seconds) {
  ...
  handle.pcm_rx = pcm_open(adev->snd_card, pcm_dev_rx_id,
                           (PCM_OUT | PCM_MONOTONIC), &pcm_config_cirrus_rx);
  if (!handle.pcm_rx) {
  ...
  if (silence == NULL) {
    ALOGE("%s: Cannot allocate %d bytes: Memory exhausted.", __func__,
          frames_bytes);
    goto exit;
  }
  ...
  ALOGD("%s: Start playing silence audio: sec=%d, count=%d", __func__, seconds,
        silence_cnt);
...
static int cirrus_do_reset(const char *channel) {
  ...
  ret = cirrus_format_mixer_name("CCM Reset", channel, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
...
static int cirrus_exec_fw_download(const char *fw_type, const char *channel,
  ...
  ret = cirrus_format_mixer_name("DSP Booted", channel, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ret = cirrus_get_mixer_value_by_name(ctl_name);
  ...
  ret = cirrus_format_mixer_name("DSP1 Preload Switch", channel, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ret = cirrus_set_mixer_value_by_name(ctl_name, 0);
  ...
  ret = cirrus_format_mixer_name("DSP1 Firmware", channel, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name("PCM Source", channel, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  if (!strcmp(fw_type, "Protection")) {
    ret = cirrus_format_mixer_name("DSP1 Protection cd CSPL_ENABLE", channel,
                                   ctl_name, sizeof(ctl_name));
  } else if (!strcmp(fw_type, "Calibration")) {
    ret = cirrus_format_mixer_name("DSP1 Calibration cd CSPL_ENABLE", channel,
                                   ctl_name, sizeof(ctl_name));
  } else {
    ret = -EINVAL;
...
retry_fw:
  ...
  if ((cspl_ena[0] + cspl_ena[1] + cspl_ena[2]) == 0 && cspl_ena[3] == 1) {
    ...
    ALOGE("%s: Firmware download failure. CSPL Status: %u %u %u %u", __func__,
...
static int cirrus_mono_calibration(void) {
  ...
  ret = cirrus_set_mixer_array_by_name(CIRRUS_CTL_CALI_CAL_AMBIENT, cal_ambient,
                                       4);
  if (ret < 0) {
  ...
  if (ret < 0)
    ALOGW("%s: Playing silence went wrong, calibration may fail...", __func__);
  ...
  ALOGD("%s: Diagnostics -- "
        ...
        cal_diag.diag_f0[3], cal_diag.diag_f0_status[0],
        cal_diag.diag_f0_status[1], cal_diag.diag_f0_status[2],
        cal_diag.diag_f0_status[3], cal_diag.diag_z_low_diff[0],
        cal_diag.diag_z_low_diff[1], cal_diag.diag_z_low_diff[2],
        cal_diag.diag_z_low_diff[3]);
#endif
  ...
  ALOGE("%s: DEBUG! status: 0x%x 0x%x 0x%x 0x%x  "
        ...
        handle.spkr.status[2], handle.spkr.status[3], handle.spkr.checksum[0],
        handle.spkr.checksum[1], handle.spkr.checksum[2],
        handle.spkr.checksum[3], handle.spkr.cal_r[0], handle.spkr.cal_r[1],
...
static int cirrus_stereo_calibration(void) {
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_CALI_CAL_STATUS, "L", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_CALI_CAL_STATUS, "R", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_CALI_CAL_CHECKSUM, "L", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_CALI_CAL_CHECKSUM, "R", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_CALI_CAL_R, "L", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_CALI_CAL_R, "R", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
...
static int cirrus_write_cal_checksum(struct cirrus_cal_result_t *cal,
                                     char *lr) {
  char ctl_name[CIRRUS_CTL_NAME_BUF];
  int ret;

  ret = cirrus_format_mixer_name(CIRRUS_CTL_PROT_CAL_CHECKSUM, lr, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
   ...
   */
  ret = cirrus_format_mixer_name(CIRRUS_CTL_PROT_CAL_CHECKSUM_CD, lr, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
...
static int cirrus_write_cal_status(struct cirrus_cal_result_t *cal, char *lr) {
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_PROT_CAL_STATUS, lr, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_PROT_CAL_STATUS_CD, lr, ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
...
static int cirrus_do_fw_stereo_download(int do_reset) {
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_PROT_CAL_R, "R", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_PROT_CAL_R, "L", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
...
static int
cirrus_do_fw_calibration_download(struct cirrus_playback_session *hdl) {
  int ret = 0;
...
static void *cirrus_do_calibration() {
  ...
  ALOGI("%s: Calibrating with ambient values 0x%x 0x%x 0x%x 0x%x", __func__,
        cal_ambient[0], cal_ambient[1], cal_ambient[2], cal_ambient[3]);
...
static int cirrus_check_error_state_stereo(void) {
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_PROT_CSPL_ERRORNO, "L", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ...
  ret = cirrus_format_mixer_name(CIRRUS_CTL_PROT_CSPL_ERRORNO, "R", ctl_name,
                                 sizeof(ctl_name));
  if (ret < 0)
    return ret;
  ret = cirrus_get_mixer_array_by_name(ctl_name, &cspl_error, 4);
...
static void cs35l41_enable_output(struct audio_device *adev,
...
static void cs35l41_disable_output(struct audio_device *adev,
                                  snd_device_t snd_device) {
  ALOGV("%s: Entry", __func__);
  ...
  if (handle.state != PLAYBACK) {
    ALOGV("%s: Cannot stop processing, state is not PLAYBACK (but %d)",
...
static int amp_enable_devices(UNUSED struct amplifier_device *device,
...
static int amp_calib(void *adev) {
  ...
  ret = get_ta_array(TA_CIRRUS_CAL_SPKL_CAL_CHECKSUM, &handle.spkl.checksum,
                     true);
  if (ret)
    return -EINVAL;
  ...
  ret = get_ta_array(TA_CIRRUS_CAL_SPKR_CAL_CHECKSUM, &handle.spkr.checksum,
                     true);
  if (ret)
    return -EINVAL;
#endif
...
static int amp_module_open(const hw_module_t *module, const char *name,
  ...
  cs35l41_dev->amp_dev.amp_calib = amp_calib;
